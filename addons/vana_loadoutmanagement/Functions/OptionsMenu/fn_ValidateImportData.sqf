...
_ValidateArray =
{
	params ["_Depth","_MaxDepth","_Result","_InString","_ArrayDataString"];
	...
	_Result = [];
	_InString = False;
	_ArrayDataString = _This splitString "";
...
{
	private _Result = _This call _ValidateArray;
...
_LoadoutDataValidation = //Format Validation
{
	...
	_Result = _This call _ValidateArray;
...

{
	...
	{
		_Result = _x call _LayoutDataValidation;
		if (count _Result > 0) then {_DataTypes Pushback "Layout"; _LayoutData = _Result};
	};

	if (_LoadoutData isequalto []) then
	{
		_Result = _x call _LoadoutDataValidation;
		if (count _Result > 0) then {_DataTypes Pushback "Loadout"; _LoadoutData = _Result};
	};
} foreach _Import;
